...
type logConfig struct {
	...
	Console    *consoleLogger `json:"Console,omitempty" mapstructure:"Console"`
	File       *fileLogger    `json:"File,omitempty" mapstructure:"File"`
	Conn       *connLogger    `json:"Conn,omitempty" mapstructure:"Conn"`
}
...
func (this *LocalLogger) Fatal(format string, args ...interface{}) {
	...
	this.Emer("###Exec Exit:"+format, args...)
	os.Exit(1)
}
...
// param 可以是log配置文件名，也可以是map类型的配置内容,默认DEBUG输出到控制台
func SetLogger(param ...interface{}) error {
	if 0 == len(param) {
		//默认只输出到控制台
	...
	switch ty.Kind() {
	case reflect.String:
		err := json.Unmarshal([]byte(c.(string)), conf)
		if err != nil { //不是json，就认为是配置文件，如果都不是，打印日志，然后退出
			// Open the configuration file
			fd, err := os.Open(c.(string))
			if err != nil {
	...
	case reflect.Map:
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "the configuration does not match the expected map : %s\n", c, err)
			os.Exit(1)
			return err
		}
	}
