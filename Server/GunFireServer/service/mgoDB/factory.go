...
// key：数据库关键字
// conf：配置
func (f *MgoFactory) AddConf(key string, conf *config.MongoConf) error {
	f.Lock()
	defer f.Unlock()
...
// 添加secondary模式的store
// conf：数据库配置文件
func createSecondary(conf *config.MongoConf) (*MgoStore, error) {
	...
	dbStore := NewStore(dbCli, conf.DbName)
	return dbStore, nil
}

// 添加primary模式的store
// conf：数据库配置文件
//func createPrimary(conf *config.MongoConf) (*MgoStore, error) {
//	var dbCli *mongo.Client
